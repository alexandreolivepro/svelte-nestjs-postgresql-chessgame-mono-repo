...
const customGameStore = {
	...
	addMove: (move: string) => {
		gameStore.update((game) => {
			if (
				game.availableMoves?.find((availableMove) => availableMove === +move) &&
				game.selectedPiece
			) {
				game.moves.push({
				...
				if (game.board.findIndex((piece) => piece.position === +move) > -1) {
					game.board.splice(
						game.board.findIndex((piece) => piece.position === +move),
						1
					);
				}
				// Move the selected piece at the destination
				game.board[
					game.board.findIndex((piece) => piece.position === game.selectedPiece?.position)
				].position = +move as Position;
	...
	setSelectedPiece: (piece: ChessPiece) => {
		gameStore.update((game) => {
			game.availableMoves =
				game.selectedPiece?.position === piece.position
					? []
					: piece.getAvailablePositions(game.board, game.moves, true);
