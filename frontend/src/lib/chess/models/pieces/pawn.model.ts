...
export class Pawn extends ChessPieceAbstract {
	...
	getAvailablePositions(pieces: ChessPiece[], moves: Move[], isMovedPiece: boolean): Position[] {
		...
		const hasPieceInPossibleMovement = pieces.find(
			(piece) => piece.position === ((this.position + direction) as Position)
		);
		if (!hasPieceInPossibleMovement) {
			availableMoves.push((this.position + direction) as Position);
			if (
				this.isStartingPosition() &&
				!pieces.find((piece) => piece.position === ((this.position + direction * 2) as Position))
			) {
	...
	onMoveAction(gameStore: GameStore): GameStore {
		...
		if (
			lastMove.piece.type === this.type &&
			lastMove.piece.color === this.color &&
			this.isEnPassantSituation(moves.slice(0, moves.length - 1), lastMove.start)
		) {
			const direction: number = movementDirection[this.color];
			const positionPawnToRemove = gameStore.board.findIndex(
				(piece) => piece.position === this.position - direction
			);
	...
	private isEnPassantSituation(moves: Move[], position: Position) {
		...
		return (
			(this.color === PieceColor.WHITE &&
				getLastDigitOfPosition(position) === '5' &&
				getLastDigitOfPosition(lastMove?.end) === '5' &&
				getLastDigitOfPosition(lastMove?.start) === '7') ||
			(this.color === PieceColor.BLACK &&
				getLastDigitOfPosition(position) === '4' &&
				getLastDigitOfPosition(lastMove?.end) === '4' &&
				getLastDigitOfPosition(lastMove?.start) === '2')
		);
	}

	private hasOpponentPieceAtPosition(
		pieces: ChessPiece[],
		direction: number,
		modifier: number
	): boolean {
		return !!pieces.find(
			(piece) =>
				piece.position === ((this.position + modifier * direction) as Position) &&
				piece.color === getOppositeColor(this.color)
		);
	}

	private isStartingPosition(): boolean {
		return (
			(this.color === PieceColor.BLACK && this.position.toString()[1] === '7') ||
