...
export default class King extends ChessPieceAbstract {
	...
	filterKingMovesIfHeIsChecked(board: ChessPiece[], availableMoves: Position[]) {
		...
		const kingIndex = board.findIndex(
			(piece) => piece.type === PieceType.KING && piece.color === this.color
		);
		const boardWithoutKing = board.slice(0, kingIndex).concat(board.slice(kingIndex + 1));
		const piecesAttackingTheKingMoves = piecesAttackingTheKing
			.map((piece) => piece.getAvailablePositions(boardWithoutKing, [], false))
			.flat();
		return availableMoves.filter(
			(move) => !piecesAttackingTheKingMoves.find((position) => position === move)
		);
	}
	...
	removeImpossibleMoves(
		...
		const enemieAvailablePositions = board
			.filter((piece) => piece.color === getOppositeColor(this.color))
			.map((piece) => piece.availableMoves)
			.flat();

		let newAvailableMoves = availableMoves.filter(
			(position) => !enemieAvailablePositions.find((p) => p === position)
		);

		newAvailableMoves = this.removeAvailableMovesAttackingPiecesDefendedByOtherPieces(
			board,
			availableMoves
		);
		...
		return newAvailableMoves.filter((availableMove) => {
			return !(
				this.checkEmptySquareBetweenKingAndDestinationCastle(
					availableMove,
					'long',
					newAvailableMoves
				) ||
	 ...
	 */
	removeAvailableMovesAttackingPiecesDefendedByOtherPieces(
		board: ChessPiece[],
		availableMoves: Position[]
	): Position[] {
		return availableMoves.filter((move) => {
			...
			if (hasPieceOnPossibleMove > -1) {
				newBoard = sliceArray<ChessPiece>(
					newBoard,
					newBoard.findIndex((piece) => piece.position === move)
				);
			}
	 ...
	 */
	checkEmptySquareBetweenKingAndDestinationCastle(
		...
		return (
			availableMove === castlePosition[castleType].kingDestination &&
		...
		Object.keys(castlePosition).forEach((name) => {
			...
			const castleTowerAtStartingPosition = gameStore.board.findIndex(
				(piece) =>
					...
					piece.position === castle.towerDefaultPosition &&
					piece.name === name
			);
			if (
				castleTowerAtStartingPosition > -1 &&
				!this.hasRookAlreadyMoved(
					gameStore.board[castleTowerAtStartingPosition] as Rook,
					gameStore.moves
				) &&
				this.position === castle.kingDestination
			) {
				gameStore.board[castleTowerAtStartingPosition].position =
					castle.towerDestination as Position;
			}
		});
		return gameStore;
	}
			...
			Object.keys(castlePosition).forEach((name) => {
				const castleTowerAtStartingPosition = pieces.find(
					(piece) =>
						piece.color === this.color &&
						piece.type === PieceType.ROOK &&
						piece.position === castlePosition[name].towerDefaultPosition &&
						piece.name === name
				) as Rook;
				if (
					castleTowerAtStartingPosition &&
					!this.hasRookAlreadyMoved(castleTowerAtStartingPosition, moves)
				) {
