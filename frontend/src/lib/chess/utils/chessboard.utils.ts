 ...
 * @returns 
 */
export function getAvailableMovesBySide(pieces: ChessPiece[], movementValuesBySide: number[], startingPosition: Position, color: PieceColor, distance = 8): Position[] {
...
export function getStraigthLineBetweenTwoPieces(firstPiece: ChessPiece, secondPiece: ChessPiece) {
    const highValue = secondPiece.position < firstPiece.position ? firstPiece.position : secondPiece.position;
    const lowValue = secondPiece.position < firstPiece.position ? secondPiece.position : firstPiece.position;
    let modifier = 1;
...
export function getCheckStatus(board: ChessPiece[], colorToPlay: PieceColor): CheckStatus | null {
    const king = board.find((piece) => piece.type === PieceType.KING && piece.color === colorToPlay) as King;
