...
describe('chessboard.utils', () => {
    ...
    describe('getAvailableMovesBySide(:pieces, :movementValuesBySide, :startingPosition, :color, :distance)', () => {
        it('Should not return any position because it is the edge of the board', () => {
            ...
            expect(getAvailableMovesBySide([], [-9, -10, -11], 15, PieceColor.WHITE, 8).length).toEqual(0);
        });
        ...
        it('Should return the straight line without blocking', () => {
            ...
            expect(straightMiddle).toMatchObject([55, 65, 75, 85, 46, 47, 48, 44, 43, 42, 41, 35, 25, 15]);
        ...
        it('Should be blocked by enemies but still return their position', () => {
            const mockBoard = [
                new Bishop(PieceColor.WHITE, 56),
                new Bishop(PieceColor.WHITE, 52)
            ];
            const moveBishop = getAvailableMovesBySide(mockBoard, [11, 9, -9, -11], 34, PieceColor.BLACK, 8);
        ...
        it('Should be blocked by friends but don\'t return their position', () => {
            ...
            const moveBishop = getAvailableMovesBySide(mockBoard, [11, 9, -9, -11], 34, PieceColor.WHITE, 8);
    ...
    describe('getDiagonalBetweenTwoPiece(:firstPiece, :secondPiece)', () => {
        it('Should get the correct value between 2 pieces', () => {
            expect(getDiagonalBetweenTwoPiece(new Bishop(PieceColor.WHITE, 36), new Bishop(PieceColor.WHITE, 63))).toMatchObject([63, 54, 45]);
            expect(getDiagonalBetweenTwoPiece(new Bishop(PieceColor.WHITE, 63), new Bishop(PieceColor.WHITE, 36))).toMatchObject([36, 45, 54]);
            expect(getDiagonalBetweenTwoPiece(new Bishop(PieceColor.WHITE, 37), new Bishop(PieceColor.WHITE, 63))).toMatchObject([]);
            expect(getDiagonalBetweenTwoPiece(new Bishop(PieceColor.WHITE, 25), new Bishop(PieceColor.WHITE, 61))).toMatchObject([61, 52, 43, 34]);
        });
    });

    describe('getStraigthLineBetweenTwoPieces(:firstPiece, :secondPiece)', () => {
        it('Should get the correct value between 2 pieces', () => {
            expect(getStraigthLineBetweenTwoPieces(new Bishop(PieceColor.WHITE, 36), new Bishop(PieceColor.WHITE, 66))).toMatchObject([36, 46, 56, 66]);
            expect(getStraigthLineBetweenTwoPieces(new Bishop(PieceColor.WHITE, 66), new Bishop(PieceColor.WHITE, 36))).toMatchObject([36, 46, 56, 66]);
            expect(getStraigthLineBetweenTwoPieces(new Bishop(PieceColor.WHITE, 25), new Bishop(PieceColor.WHITE, 28))).toMatchObject([25, 26, 27, 28]);
        });
    });
    ...
    describe('getPieceAttackingTheKing(:king, :board)', () => {
        it('Should return the bishop attacking the king', () => {
            ...
            const pieceAttackingTheKing = getPieceAttackingTheKing(new King(PieceColor.BLACK, 33), mockBoard);
    ...
    describe('getBoardWithoutPiece(:board, :piece)', () => {
        it('Should return the board without the specified piece', () => {
            ...
            const boardWithoutPiece = getBoardWithoutPiece(mockBoard, new Rook(PieceColor.BLACK, 38, 'long'));
    ...
    describe('filterAvailableMovesIfKingIsChecked(:board, :currentPiece, :availableMoves)', () => {
        it('Should return an empty array if the king is attacked by 2 or more pieces', () => {
            ...
            expect(filterAvailableMovesIfKingIsChecked(mockBoard, new Bishop(PieceColor.WHITE, 25), [25, 34, 43])).toEqual([]);
        });
        ...
        it('Should return the availableMoves array if the king is not under attack', () => {
            ...
            expect(filterAvailableMovesIfKingIsChecked(mockBoard, new Bishop(PieceColor.WHITE, 25), [25, 34, 43])).toEqual([25, 34, 43]);
        });
        ...
        it('Should return only the availableMoves that protect the king', () => {
            ...
            const availableMoves = [55, 47, 48, 45, 44, 43, 42, 41, 36, 26, 16, 37, 28, 35, 24, 13] as Position[];
            expect(filterAvailableMovesIfKingIsChecked(mockBoard, new Queen(PieceColor.WHITE, 46), availableMoves)).toEqual([55, 44]);
            ...
            expect(filterAvailableMovesIfKingIsChecked(mockBoard2, new Queen(PieceColor.WHITE, 46), availableMoves)).toEqual([36, 37, 35]);
        });
    });
});